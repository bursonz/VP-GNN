...
class TransformerConv(MessagePassing):
    ...
    def forward(self, x: Union[Tensor, PairTensor], edge_index: Adj, edge_weights=None,
                edge_attr: OptTensor = None, return_attention_weights=None):
        # type: (Union[Tensor, PairTensor], Tensor, OptTensor, None, None) -> Tensor  # noqa
        # type: (Union[Tensor, PairTensor], SparseTensor, OptTensor, None) -> Tensor  # noqa
...
class Raindrop(nn.Module):
    ...
    def __init__(self, d_inp=36, d_model=64, nhead=4, nhid=128, nlayers=2, dropout=0.3, max_len=215, d_static=9,
                 MAX=100, perc=0.5, aggreg='mean', n_classes=2, sensor_wise_mask=False, static=True):
        super().__init__()
        from torch.nn import TransformerEncoder, TransformerEncoderLayer
        self.model_type = 'Transformer'

        self.global_structure = torch.ones(d_inp, d_inp)
        ...
        self.encoder = nn.Linear(d_inp*self.d_ob, d_model)
        d_pe = d_model - d_inp*self.d_ob
    ...
    def forward(self, src, times, static, lengths):
        """Input to the model:
        ...
        pe = self.pos_encoder(times.squeeze(2))
