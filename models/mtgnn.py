...
class MTGNN(nn.Module):
    def __init__(self, gcn_true, buildA_true, gcn_depth, num_nodes, predefined_A=None, num_static_features=0, static_feat=None,
    ...
    def forward(self, Xtemp, Xtimes, Xstatic, lengths, idx=None):
        Xtemp = Xtemp[:, :, :int(Xtemp.shape[2] / 2)]
        Xtemp = torch.permute(Xtemp, (1, 0, 2))
        Xtemp = torch.unsqueeze(Xtemp, dim=1)
        Xtemp = Xtemp.transpose(2, 3)

        
        seq_len = Xtemp.size(3)
        assert seq_len==self.seq_length, 'Xtemp sequence length not equal to preset sequence length'

        if self.seq_length<self.receptive_field:
            Xtemp = nn.functional.pad(Xtemp,(self.receptive_field-self.seq_length,0,0,0))

        if self.gcn_true:
            if self.buildA_true:
                if idx is None:
                    adp = self.gc(self.idx.to(Xtemp.device))
                else:
                    adp = self.gc(idx)
            else:
                adp = self.predefined_A

        x = self.start_conv(Xtemp)
        skip = self.skip0(F.dropout(Xtemp, self.dropout, training=self.training))
